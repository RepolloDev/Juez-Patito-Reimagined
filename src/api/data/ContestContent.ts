...
import { BASE_FETCH_URL } from 'tests/api/constants'
...
export async function getContestContent(session: Session, contestId: string) {
  const CACHE_KEY = `contest-content-${contestId}`
  ...
  const data = accesible
    ? scrapeAccessContest(html)
    : scrapeNotAccessContest(html)
